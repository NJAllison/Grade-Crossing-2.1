...
#define VSServo true
  ...
  #define ServoSpeedDown 5
  #define ServoSpeedUp 55
#else
  #include <Servo.h>
#endif
// *************************************************************************
...
XingDef Crossing [NumXings] = {  AlgType, A0,                A2,                A1,               2,         4,         5,      9, 10,                  A6,A7, A6,A7,     SrvoType};
//                              SnsrType, EastSnsr[NumTrks], WestSnsr[NumTrks], XingSnsr[NumTrks],FirstLite, SoundCntl, ArmTip, GateCntl,      GatePots Up,Dwn,Up,Dwn     GateType 

// and the WaitABitSecs array
// East wait, West wait
int WaitABitSecs[NumXings][NumTrks*2] = {0,0};
//
...
// ============= LowerGate ======================
  ...
  switch (Crossing[Xidx].GateType){
    case SrvoType:
      ...
      #if VSServo
        CrossingGate[Xidx][Lidx].write(map(GateDownS, 0, 1023, 0, FullArc), ServoSpeedDown, false);
      #else
  ...
  switch (Crossing[Xidx].GateType){
    case SrvoType:
      ...
      if (Crossing[Xidx].GatePots[Sidx] != 0){
         GateDownS = analogRead(Crossing[Xidx].GatePots[Sidx]);}
      #if VSServo
        CrossingGate[Xidx][Lidx].write(map(GateUpS, 0, 1023, 0, FullArc), ServoSpeedUp, false);
      #else
...
// ============= StopLites ======================
  ...
  if (Crossing[Xidx].FirstLite > 0) {
      ...
      digitalWrite(Crossing[Xidx].FirstLite+1, LampOff);
      digitalWrite(Crossing[Xidx].ArmTip,   LampOff); 
  } // if FirstLite is defined
  if (Crossing[Xidx].SoundCntl > 0) {
...
//  SnsrType, SnsrLst[NumSnsrs], FirstLite, SoundCntl, GateCntl
      ...
      pinMode(Crossing[Xidx].ArmTip, OUTPUT);
      digitalWrite(Crossing[Xidx].FirstLite,   LampOff);
      digitalWrite(Crossing[Xidx].FirstLite+1, LampOff);
      digitalWrite(Crossing[Xidx].ArmTip,   LampOff); 
    } // FirstLite
